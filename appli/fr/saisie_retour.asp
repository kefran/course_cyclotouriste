...
if isNull(rsCourse("DECOMPTE")) then
	Session("strError")="La course n'a pas été démarrée! Vous ne pouvez donc pas saisir de retours."
	response.redirect "index_admin.asp"
elseif rsCourse("DECOMPTE")="Faux" then
	Session("strError")="La course a déjà eu lieu! Vous devez créer une nouvelle course pour pouvoir saisir des retours."
	response.redirect "index_admin.asp"
end if
...
if request.querystring("search")>0 then
	...
	rsSearch.Open "Select NUMCYC from CYCLISTE WHERE NUMCYC IN (SELECT NUMCYC FROM PARTICIPER WHERE NUMCOURSE=" & intNumcourse & " AND HDEPART IS NOT NULL AND HARRIVEE IS NULL) AND NUMCYC=" & request.querystring("search"),Conn,adOpenForwardOnly,adLockReadOnly
	if rsSearch.EOF then
		Session("strError")="Le cycliste recherché n'est pas encore parti ou est déjà arrivé!"
		response.redirect "saisie_retour.asp"
	else
...
function chercher_cycliste(num) { 
	if (num!="")
	{
  	var url='saisie_retour.asp?search=' + num; 
...
{
	var url='saisie_retour.asp?numcyc=' + document.form0.cbnom.value; 
	window.location.replace(url);
}
...
if (intNumcyc>0 and blnNoMdif=false) then
	%>
	<input type="button" value="Enregister l'arrivée" onclick="document.form1.submit();">
	<%
else
	%>
	<input type="button" value="Enregister l'arrivée" disabled>
	<%
end if
%>
<input type="button" value="Imprimer Diplôme" onclick="window.open('action_diplomes.asp?numcyc=<% =intNumcyc %>');">	

<% 
if intNumcyc>0 then
	%>
	<input type="button" value="Modifier le cycliste" onclick="window.location.replace('edit_cycliste.asp?from=retour&mode=edit&numedit=<% =intNumcyc %>');">
	<%
else
	%>
...
<table border="0">
	<tr>
		...
		Dim intNBC,strHDEPART,strHARRIVEE,strTEMPS
		if rsCyc.EOF then
			intNBC=0
			strHDEPART=""
			strHARRIVEE=""
			...
			strHARRIVEE=DateConvert(rsCyc("HARRIVEE"))
		end if
		
		if strHDEPART<>"" AND strHARRIVEE<>"" then
			strTEMPS=CDate(CDate(strHDEPART)-CDate(strHARRIVEE))
		else
			strTEMPS=""
		end if
		
		if strHDEPART="" OR (strHDEPART<>"" AND strHARRIVEE<>"") then
			blnNoMdif=true
		end if
		...
		Dim strDistance
		if intNBC=1 then
			strDistance="1 - " & rsCyc("DISTANCEC1") & "km"
		elseif intNBC=2 then
			strDistance="2 - " & rsCyc("DISTANCEC2") & "km"
		elseif intNBC=3 then
			strDistance="3 - " & rsCyc("DISTANCEC3") & "km"
		else
			strDistance=""
		end if
		%>
		<input type="text" name="distance" size="9" maxlength="9" value="<% =strDistance %>" readonly style="background: #e6e6e6 ; font: bold">
		<b>		
		
		&nbsp;&nbsp;
		Départ:&nbsp;&nbsp;
		<input type="text" name="hdepart" size="8" maxlength="8" readonly style="background: #e6e6e6; font: bold" value="<% =strHDEPART	%>">
		&nbsp;&nbsp;
		Retour:&nbsp;&nbsp;
		<input type="text" name="harrivee" size="8" maxlength="8" readonly style="background: #e6e6e6; font: bold" value="<% =strHARRIVEE	%>">
		Tps:&nbsp;&nbsp;
		<input type="text" name="temps" size="8" maxlength="8" readonly style="background: #e6e6e6; font: bold" value="<% =strTEMPS	%>">
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<%
		rsCyc.close
...
if (intNumcyc>0 and blnNoMdif=false) then
	%>
	<input type="submit" value="Enregister l'arrivée" >
	<%
else
	%>
	<input type="button" value="Enregister l'arrivée" disabled>
